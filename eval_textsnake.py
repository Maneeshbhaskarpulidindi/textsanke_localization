...
def write_to_file(contours, file_path):
    ...
    with open(file_path, 'w') as f:
        for cont in contours:
            ...
            cont = cont.flatten().astype(int).astype(str).tolist()
            cont = ','.join(cont)
            f.write(cont + '\n')
...
def inference(detector, test_loader, output_dir):
    ...
    for i, (image, train_mask, tr_mask, tcl_mask, radius_map, sin_map, cos_map, instance_labels, meta) in enumerate(test_loader):

        image, train_mask, tr_mask, tcl_mask, radius_map, sin_map, cos_map, instance_labels = to_device(
            image, train_mask, tr_mask, tcl_mask, radius_map, sin_map, cos_map, instance_labels)
        ...
        pred_vis = visualize_detection_mod(img_show, contours, tr_pred[1], tcl_pred[1])
        gt_contour = []
        for annot, n_annot in zip(meta['annotation'][idx], meta['n_annotation'][idx]):
            if n_annot.item() > 0:
                gt_contour.append(annot[:n_annot].int().cpu().numpy())
        gt_vis = visualize_detection_mod(img_show, gt_contour, tr_mask[idx].cpu().numpy(), tcl_mask[idx].cpu().numpy())
...
def main():
    ...
    testset = TotalText(
        data_root='/home/frinksserver/subhra/OCR/TextSnake.pytorch/data/total-text',
        ignore_list=None,
        is_training=False,
