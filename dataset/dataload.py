...
class TextDataset(data.Dataset):
    ...
    def fill_polygon(self, mask, polygon, value):
        ...
        """
        cv2.fillPoly(mask, [polygon.astype(np.int32)], color=(value,))
    ...
    def get_training_data(self, image, polygons, image_id, image_path):
        ...
        H, W, _ = image.shape
        polygons = [p for p in polygons if p.text != '###'] ## filter out polygoins with ### marking transcript
        ...
        instance_label_map = np.zeros(image.shape[:2], np.int32)   # NEW: per-pixel instance IDs

        for i, polygon in enumerate(polygons):
            instance_id = i + 1   # 0 = background, start from 1
            # if polygon.text != '###':
            ...
            # Assign instance ID to the TCL pixels just drawn for this polygon
            newly_drawn = (tcl_mask > 0) & (instance_label_map == 0)
            instance_label_map[newly_drawn] = instance_id
        ...
        return image, train_mask, tr_mask, tcl_mask, radius_map, sin_map, cos_map, instance_label_map, meta
    ...
    def get_test_data(self, image, image_id, image_path):  ### edited by subhra
        """
        Prepare test data with proper image format and metadata.
        Handles grayscale and BGRA images, ensuring they are in BGR format.
        """
        import cv2
        import numpy as np
        ...
        # Normalize channel shape to 3-channel BGR
        if image.ndim == 2:
            # Grayscale -> BGR
            image = cv2.cvtColor(image, cv2.COLOR_GRAY2BGR)
        elif image.ndim == 3:
            ...
            if c == 1:
                ...
                raise ValueError(f"Unsupported channel count {c} for image: {image_path}")
        else:
            raise ValueError(f"Unsupported image shape {image.shape} for: {image_path}")

        # Ensure contiguous memory layout (some transforms expect this)
        image = np.ascontiguousarray(image)

        # Meta BEFORE transform (matches original behavior)
        H, W, _ = image.shape
        meta = {
            'image_id': image_id,
            'image_path': image_path,
        ...
        # Apply transform (handle both return styles)
        if self.transform:
            out = self.transform(image)
            image = out[0] if isinstance(out, (tuple, list)) else out

            # If transform accidentally returned grayscale, fix again
            if image.ndim == 2:
                image = cv2.cvtColor(image, cv2.COLOR_GRAY2BGR)

        # To PyTorch channel order (C, H, W)
        image = image.transpose(2, 0, 1)
